...
#'
#' @param fn.plot string, filename for the plot (any extension supported by ggsave
#' is allowed). If \code{fn.plot} is \code{NULL}, the plot will only be returned as a ggplot object.
#'
#' @param color.scheme valid R color scheme or vector of valid R colors (must 
#' be of length 3 for positice, negative, and non-significant associations),
...
#' 
#' @param annot.y.exp float, percent vertical expansion of the plot area to accomodate annotations
#' 
#' @param annot.y.shift float, upward shift of the annotations from the respective datapoints
#' 
...
volcano.plot <- function(
    siamcat, fn.plot=NULL, color.scheme=c('red', 'blue', 'gray'), annotate=3,
    ...
    if (length(color.scheme) == 1) {
        tryCatch({col <- brewer.pal(3, color.scheme)[c(1, 3, 2)]}, error=function(e){stop(
        ...
        tryCatch(col2rgb(color.scheme), error=function(e){stop(
            "color.scheme contains 3 elements, so it is interpreted ",
            "as containing individual colors, but the color names are invalid."
        )})
        col <- color.scheme
    } else {
        stop("color.scheme must contain 3 R color values or the name of 1 ColorBrewer palette.")
    }
    ...
    plot <- ggplot(associations, aes(x=eff, y=-log10(p.adj), size=pr.all, fill=class, label=label)) +
        geom_point(shape=21, alpha=0.3) +
        ggrepel::geom_text_repel(
            data=associations.to.label, ylim=c(max(-log10(associations$p.adj)), max(-log10(associations$p.adj))*(1+annot.y.exp)),
            show.legend=FALSE, size=annot.size, force=20, force_pull=0
        ) +
        geom_hline(
            yintercept=-log10(assoc.param$alpha), color='gray', lty='dashed', lwd=0.5
        ) +
        annotate(
            "text", x = Inf, y = -log10(assoc.param$alpha), label = deparse(bquote(alpha~"="~.(assoc.param$alpha))),
            color = "gray40", parse=TRUE, hjust=1, vjust=-1
        ) +
        scale_fill_manual(values=col, breaks=names(col), guide=guide_legend(override.aes = list(size = 6))) +
        scale_size(guide = guide_legend(reverse = TRUE)) +
        labs(x=xlab, y=bquote(-log[10](italic(.(mult.corr.label)))), size="Prevalence", fill=fill_lab) +
        theme_grey(font.size) +
        theme(
            line = element_line(
                color="black", linetype=1, lineend="butt", linewidth=line.size
            ),
            text = element_text(
            ...
            axis.text.x       = element_text(margin = margin(t = small.size / 4), vjust = 1),
            axis.text.y       = element_text(margin = margin(r = small.size / 4), hjust = 1),
            axis.ticks        = element_line(color = "black", linewidth = line.size),
            axis.ticks.length = unit(half.line / 2, "pt"),
            axis.title.x      = element_text(margin = margin(t = half.line / 2), vjust = 1),
            axis.title.y      = element_text(angle = 90, margin = margin(r = half.line / 2), vjust = 1),
    ...
    if (nrow(associations.to.label) != 0){
        plot <- plot + scale_y_continuous(expand=expansion(mult=c(0.05, annot.y.exp)))
    }

    # save the plot
    if (!is.null(fn.plot)){
        ggsave(fn.plot, plot, bg="white", height = 6, width = 7, device='png')
    }
